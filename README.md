wait(0.1)game.StarterGui:SetCore("SendNotification",{Title="cozgent.xyz",Text="cozgent.xyz Successfully loaded!",Duration=7})
getgenv().values = {}
...
local gui = library:New("CozGent v1")      
...
client:Element("Toggle", "Spectators List", nil, function(tbl)      
	if tbl.Toggle then      
        ...
        Color.BackgroundColor3 = Color3.fromRGB(100,149,237)
...
client:Element("Toggle", "No clip", nil, function(tbl)      
  tbl = false
  if tbl == false then
    noclip = false
    ...
    print('Loaded')
    print('Press "E" to noclip')    print(value)
...
local KeystrokesToggle = false

client:Element("Toggle", "Keystrokes", nil, function(tbl)      
	if KeystrokesToggle == false then
...
chat:Element("Toggle", "chat spam", nil, function(tbl)      
	if tbl.Toggle then      
		while values.misc.chat["chat spam"].Toggle do      
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "Cozmic and Agent on t0p", false, "Innocent", false, true)      
...
CreateHitElement("Loading Cozgent..",Color3.new(1,1,0))
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "nn fue destruido por Hexware. propiedad de p10000000 cheat (Cumhook)", false, "Innocent", false, true)      
	end      
end)      
